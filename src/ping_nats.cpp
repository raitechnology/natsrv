...
main( int argc, char **argv )
{
  ...
  const char * po = get_arg( argc, argv, 1, "-p", "4222" ),
             * us = get_arg( argc, argv, 1, "-u", NULL ),
             * pa = get_arg( argc, argv, 1, "-x", NULL ),
             * au = get_arg( argc, argv, 1, "-a", NULL ),
  ...
  if ( he != NULL ) {
    fprintf( stderr,
             "%s [-p port] [-u user] [-x pass] [-a auth_toen] [-r] [-c count]\n"
             "  -p port    = daemon port\n"
             "  -u user    = NATS user\n"
             "  -x pass    = NATS password\n"
             "  -a auth    = NATS auth_token\n"
             "  -r         = reflect pings\n"
  ...
  EvNatsClientParameters parm( "127.0.0.1", "ping", us, pa, au, atoi( po ) );
  Ping<EvNatsClientParameters, EvNatsClient> conn( poll, parm );
